...
### Setting Up the Dagster Pipeline

The Dagster pipeline processes PDF files and generates embeddings.
...
1. **Start the Dagster UI**:
   ...
   This starts the Dagster web interface at `http://localhost:3000` (or the next available port).

2. **Add PDF Files**:
   - Place your PDF files in `data/features/<folder_name>/`
   - Each folder becomes a partition in Dagster
   - Example: `data/features/sample/` contains a set of demo PDFs for testing

3. **Materialize Assets**:
   - Open the Dagster UI in your browser
   - Navigate to the "Assets" tab
   - Select a partition (folder) from the dropdown
   - Click "Materialize" to run the pipeline for that partition

   The pipeline will:
   - Discover all PDFs in the selected folder
   - Extract text from each PDF
   - Save PDF metadata to the `resources` table
   - Generate embeddings using OpenAI
   - Save embeddings to the `embeddings` table

### Setting Up the Next.js Application

The Next.js app provides the chat interface for querying documents.
...
3. **Run database migrations**:
   ...
   This sets up the database schema for the Next.js app (chats, messages, users, etc.).
...
5. **Access the application**:
   ...
   - Select a model from the dropdown (RAG model, GPT-4, or GPT-4o Reasoning)
